...
public class KlantRepository {
    ...
    public Klant findOneRecord(String Bnaam, String Gnaam) {
        ...
        try {
            String sql = "select kl.id, kl.bedrijfsnaam, kl.gebruikersnaam, kl.klantnummer, kl.postcode, p.id pid, p.naam pnaam" +
                    " from klant kl" +
                    " join persoon p"+
                    " on p.id = kl.persoon_id where kl.bedrijfsnaam = ? or kl.gebruikersnaam = ?";
    ...
    public int insertOneRecord(Klant klant) {
        ...
        int insertId = 0;
        try {
            String sql = "insert into klant (bedrijfsnaam, gebruikersnaam, klantnummer, postcode, persoon_id) values(?,?,?,?,?)";
    ...
    public int updateOneRecord(Klant klant) {
        ...
        int result = 0;
        try {
            String sql = "update klant kl set kl.bedrijfsnaam = ?, kl.persoon_id = ? where kl.id = ?";
            stmt = connection.prepareStatement(sql);
            stmt.setString(1, klant.getBedrijfsnaam());
